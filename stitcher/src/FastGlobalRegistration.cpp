...
static std::vector<std::pair<int, int>> AdvancedMatching(
    ...
    for (i = 0; i < number_of_trial; i++) {
        ...
        const auto& pti0 = src_point_cloud.points_[idi0];
        const auto& pti1 = src_point_cloud.points_[idi1];
        const auto& pti2 = src_point_cloud.points_[idi2];
        double li0 = (pti0 - pti1).norm();
        double li1 = (pti1 - pti2).norm();
        double li2 = (pti2 - pti0).norm();

        // collect 3 points from dest fragment
        const auto& ptj0 = dst_point_cloud.points_[idj0];
        const auto& ptj1 = dst_point_cloud.points_[idj1];
        const auto& ptj2 = dst_point_cloud.points_[idj2];
...
NormalizePointCloud(std::vector<geometry::PointCloud>& point_cloud_vec,
                    const FastGlobalRegistrationOption& option) {
    utility::LogDebug("begin NormalizePointCloud optimized");
    const int num = 2;
    double scale = 0;
    std::vector<Eigen::Vector3d> pcd_mean_vec(num);
    ...
    for (int i = 0; i < num; ++i) {        
        ...
        const int npti = static_cast<int>(point_cloud_vec[i].points_.size());
        for (int ii = 0; ii < npti; ++ii) {
            mean = mean + point_cloud_vec[i].points_[ii];
        }
        mean = mean / npti;
        pcd_mean_vec[i] = mean;
        utility::LogDebug("normalize points :: mean = [{:f} {:f} {:f}] thread_id: {:d}",
                          mean(0), mean(1), mean(2), omp_get_thread_num());
...
#pragma omp parallel for reduction(max : max_scale) // omp 2.0 doesn't support this
        for (int ii = 0; ii < npti; ++ii) {
            double temp = point_cloud_vec[i].points_[ii].norm();
            if (temp > max_scale) max_scale = temp;
        }

        scale = std::max(max_scale, scale);
    }
...
static Eigen::Matrix4d OptimizePairwiseRegistration(
    ...
    const int numIter = option.iteration_number_;
    ...
    for (int itr = 0; itr < numIter; itr++) {
        ...
        for (size_t c = 0; c < corres.size(); c++) {
            ...
            //Eigen::Vector3d p, q;
            const auto& p = point_cloud_vec[i].points_[ii];
            const auto& q = point_cloud_copy_j.points_[jj];
            Eigen::Vector3d rpq = p - q;

            //const size_t c2 = c;
            double temp = par / (rpq.dot(rpq) + par);
            const double s = temp * temp;
...
RegistrationResult FastGlobalRegistrationBasedOnCorrespondence(
    ...
    return EvaluateRegistration(
            source, target, option.maximum_correspondence_distance_, inv_mat);
}
